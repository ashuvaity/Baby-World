function validateForm(){
	    var n = document.form1.lname;
        var n = document.form1.fname;
		var n = document.form1.mname;
    ...
    if (atpos<1 || dotpos<atpos+2 || dotpos+2>=x.length) {
        ...
        if (mobn.value.length != 11){alert("Mobile number must be 11 digit"); return false;}
        if (!(opt1.checked) && !(opt2.checked) && !(opt3.checked))
        {
            window.alert("Provide Gender"); return false;
        }
...
function myFunction() {
    submit("you are sucessfully registerd!");
}